...
val_sdfs = ["Data/OptunaMgCdLa/Mg_val.sdf", "Data/OptunaMgCdLa/Cd_val.sdf"]
test_sdfs = ["Data/OptunaMgCdLa/Mg_test.sdf", "Data/OptunaMgCdLa/Cd_test.sdf"]
output_path = "Output"
output_mark = f"OptunaAtt"
# valuenames = ["logK_Cu"]
...
class MolGraphHeteroNetParams(MolGraphNetParams):
    def __init__(self, trial: optuna.Trial, metal_features, conv_features,
                 ...
                 metal_ligand_unifunc_variants=None,
                 attention_key_hidden_variants=None, attention_query_hidden_variants=None,
                 attention_actf_variants=None, attention_bn_variants=None,
                 num_heads_variants=(1,), **kwargs):
        ...
        self.attention_key_hidden_variants = attention_key_hidden_variants
        self.attention_query_hidden_variants = attention_query_hidden_variants
        self.attention_actf_variants = attention_actf_variants
        self.attention_bn_variants = attention_bn_variants
        self.num_heads_variants = num_heads_variants
        ...
        self.num_heads = None
        self.attention_key_hidden = None
        self.attention_query_hidden = None
        self.attention_actf = None
        self.attention_bn = None

    def get_hidden_dims(self):

        hidden_metal = [self.trial.suggest_categorical(f"metal_layer{i}", self.hidden_variants)
                        for i in range(self.n_metal)]

        if self.metal_ligand_unifunc_name in ["concat"]:
            hidden_conv = [self.trial.suggest_categorical(f"conv_layer{i}", self.hidden_variants)
                           for i in range(self.n_conv)]
            hidden_linear = [hidden_metal[-1] + hidden_conv[-1]] + \
                            [self.trial.suggest_categorical(f"linear_layer{i}", self.hidden_variants)
                             for i in range(self.n_linear)]
    ...
    def get(self):
        ...
        self.num_heads = self.trial.suggest_categorical("num_heads", self.num_heads_variants)
        use_key_query_sequential = self.trial.suggest_categorical("use_key_query_sequential", [True, False])
        self.attention_key_hidden = (self.trial.suggest_categorical("attention_key_hidden",
                                                                    self.attention_key_hidden_variants),) if use_key_query_sequential else ()
        self.attention_query_hidden = (self.trial.suggest_categorical("attention_query_hidden",
                                                                      self.attention_query_hidden_variants),) if use_key_query_sequential else ()
        attention_actf_name = self.trial.suggest_categorical("attention_actf_name",
                                                             list(self.attention_actf_variants.keys()))
        self.attention_actf = self.attention_actf_variants[attention_actf_name]
        self.attention_bn = self.trial.suggest_categorical("attention_bn", self.attention_bn_variants)
        ...
        model_parameters = {
            ...
            "optimizer_parameters": self.optimizer_parameters,
            "use_attention": True,
            "attention_name": "GlobalAddAttention",
            "attention_parameters": {"num_heads": self.num_heads},
            "attention_key_hidden": self.attention_key_hidden,
            "attention_query_hidden": self.attention_query_hidden,
            "attention_key_actf": self.attention_actf,
            "attention_query_actf": self.attention_actf,
            "attention_key_bn": self.attention_bn,
            "attention_query_bn": self.attention_bn,
...
def objective(trial: optuna.Trial):
    ...
    metal_val_sets = [featurize_sdf_with_metal(path_to_sdf=val_sdf,
                                               mol_featurizer=ConvMolFeaturizer(),
                                               metal_featurizer=SkipatomFeaturizer("Source/featurizers/skipatom_vectors_dim200.torch"))
                      for val_sdf in val_sdfs]
    metal_test_sets = [featurize_sdf_with_metal(path_to_sdf=test_sdf,
                                                mol_featurizer=ConvMolFeaturizer(),
                                                metal_featurizer=SkipatomFeaturizer("Source/featurizers/skipatom_vectors_dim200.torch"))
    ...
    test_batches = [Batch.from_data_list(test_set) for test_set in metal_test_sets]

    model_parameters = MolGraphHeteroNetParams(
        trial,
        metal_features=metal_features, conv_features=conv_features,
        n_metal_lims=(1, 4), n_conv_lims=(1, 3), n_linear_lims=(1, 4),
        ...
        attention_key_hidden_variants=(32, 64, 128),
        attention_query_hidden_variants=(32, 64, 128),
        attention_actf_variants={
            "nn.LeakyReLU()": nn.LeakyReLU(),
            "nn.PReLU()": nn.PReLU(),
            "nn.Tanhshrink()": nn.Tanhshrink()},
        attention_bn_variants=(False,),
        num_heads_variants=(1, 2, 4),
    ).get()
    ...
    try:
        ...
        test_metrics = {
            "r2_score": tuple(r2_score(batch.y, model(x=batch.x,
                                                      edge_index=batch.edge_index,
                                                      metal_x=batch.metal_x,
                                                      batch=batch.batch)) for batch in
                              test_batches),
            "mean_squared_error": tuple(mean_squared_error(batch.y, model(x=batch.x,
                                                                          edge_index=batch.edge_index,
                                                                          metal_x=batch.metal_x,
                                                                          batch=batch.batch)) for batch in
                                        test_batches),
            "mean_absolute_error": tuple(mean_absolute_error(batch.y, model(x=batch.x,
                                                                            edge_index=batch.edge_index,
                                                                            metal_x=batch.metal_x,
                                                                            batch=batch.batch)) for batch in
                                         test_batches),
        ...
        trial.set_user_attr(key="test_metrics", value=test_metrics)
        max_error = max([mean_squared_error(batch.y, model(x=batch.x,
                                                           edge_index=batch.edge_index,
                                                           metal_x=batch.metal_x,
                                                           batch=batch.batch)) for batch in val_batches])
    except ValueError:
        trial.set_user_attr(key="metrics", value=None)
        trial.set_user_attr(key="model_parameters", value=model_parameters)
        trial.set_user_attr(key="test_metrics", value=None)
        max_error = np.inf
