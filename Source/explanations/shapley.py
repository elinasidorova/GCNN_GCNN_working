...
class MarginalSubgraphDataset(Dataset):
    ...
    def __getitem__(self, idx):
        exclude_data = self.subgraph_build_func(self.data, self.exclude_mask[idx])
        include_data = self.subgraph_build_func(self.data, self.include_mask[idx])
...
def graph_build_zero_filling(graph, node_mask: np.array):
...
def graph_build_split(graph, node_mask: np.array):
...
def l_shapley(coalition: list, data: Data, local_radius: int,
              value_func, subgraph_building_method='zero_filling'):
...
def gnn_score(coalition: list, data: Data, value_func,
    ...
    mask[coalition] = 1.0
    mask_data = subgraph_build_func(data, mask)
