...
from Source.data import root_mean_squared_error, balanced_train_valid_test_split, balanced_train_valid_split
...
latent_data = {}
for test_metal in tqdm(test_metals, desc="Getting latents"):
    train_folder_name = [f for f in os.listdir(ROOT_DIR / f"Output/Uncertainty_knn_OneMOut/") if f.startswith(test_metal)][0]
    train_folder = ROOT_DIR / f"Output/Uncertainty_knn_OneMOut/{train_folder_name}"
    model = ModelShell(GCNN_FCNN, train_folder).models[0]

    latent_data[test_metal] = []
    for sample in datasets[test_metal]:
        result, latent = model(sample, return_latent=True)
        latent_data[test_metal] += [{
            "latent": latent.squeeze().detach().tolist(),
...
for test_metal in test_metals:
    ...
    output_dir = ROOT_DIR / f"Output/Uncertainty/knn_latent_OneMOut/{test_metal}"
    train_folder_name = [f for f in os.listdir(ROOT_DIR / f"Output/Uncertainty_knn_OneMOut/") if f.startswith(test_metal)][0]
    train_folder = ROOT_DIR / f"Output/Uncertainty_knn_OneMOut/{train_folder_name}"

    train_datasets = [datasets[metal] for metal in datasets if metal != test_metal]
    ...
    test_loader = DataLoader(datasets[test_metal], batch_size=batch_size)
