...
def create_folds(metals_list, n_folds, batch_size, shuffle_every_epoch, seed=17):
    ...
    for metal in metals_list:
        ...
        mol_list = featurize_sdf_with_metal_and_conditions(path_to_sdf=f"Data/OneM_cond/{metal}.sdf",
                                                           mol_featurizer=ConvMolFeaturizer(),
                                                           metal_featurizer=SkipatomFeaturizer(
                                                               "Source/featurizers/skipatom_vectors_dim200.torch"))
...
def create_train_val(metals_list, batch_size, shuffle_every_epoch, seed=17):
    ...
    for metal in metals_list:
        ...
        mol_ids = list(range(len(mol_list)))
        if len(mol_ids) < 2:
            train_index, valid_index = mol_ids, []
        else:
...
def train_testonly(test_metal, output_path, output_mark, batch_size=64, n_split=10, epochs=1000,
    ...
    test_loader = DataLoader(
        featurize_sdf_with_metal_and_conditions(path_to_sdf=f"Data/OneM_cond/{test_metal}.sdf",
                                                mol_featurizer=ConvMolFeaturizer(),
                                                metal_featurizer=SkipatomFeaturizer(
                                                    "Source/featurizers/skipatom_vectors_dim200.torch")),
        batch_size=batch_size)
...
model_parameters = {
    "hidden_metal": [203, 256, 128, 128, 64, 64],
...
train_testonly(
    ...
    output_path="Output",
    output_mark=f"WithCond/General_{test_metal}_seed{seed}_testonly_5fold",
    batch_size=32,
    n_split=5,
    epochs=1000,
    es_patience=100,
